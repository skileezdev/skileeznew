...
class ContractForm(FlaskForm):
    ...
    timezone = SelectField('Timezone', choices=get_available_timezones(), validators=[DataRequired(message='Please select a timezone')])
...
class RescheduleRequestForm(FlaskForm):
    """Form for requesting session reschedule"""
    
    reason = TextAreaField('Reason for Reschedule', validators=[
        DataRequired(message='Please provide a reason for the reschedule request'),
        Length(min=10, max=500, message='Reason must be between 10 and 500 characters')
    ])
    
    new_scheduled_at = DateTimeField('New Scheduled Time (Optional)', validators=[
        Optional()
    ], format='%Y-%m-%dT%H:%M')
...
class CoachAvailabilityForm(FlaskForm):
    ...
    timezone = SelectField('Timezone', choices=get_available_timezones(), validators=[DataRequired()])
...
class FreeConsultationForm(FlaskForm):
    ...
    timezone = SelectField('Timezone', choices=get_available_timezones(), default='UTC')
