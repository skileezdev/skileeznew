...
def create_reschedule_notification(session, notification_type, student, coach):
    """
    Create notifications for reschedule events (request/approval/decline)
    
    Args:
        session: Session object
        notification_type: 'reschedule_requested', 'reschedule_approved', or 'reschedule_declined'
    ...
    try:
        ...
        if notification_type == 'reschedule_requested':
            # Notify coach that student requested reschedule
            Notification.create_notification(
                user_id=coach.id,
                title="Reschedule Request",
                message=f"{student.first_name} {student.last_name} has requested to reschedule your session",
                notification_type='session',
                related_id=session.id,
                related_type='session'
            )
            ...
            # Notify student that reschedule was approved
            Notification.create_notification(
                user_id=student.id,
                title="Reschedule Approved",
                message=f"Your reschedule request for session with {coach.first_name} {coach.last_name} has been approved",
                notification_type='session',
                related_id=session.id,
                related_type='session'
            )
            
        elif notification_type == 'reschedule_declined':
            # Notify student that reschedule was declined
            Notification.create_notification(
                user_id=student.id,
                title="Reschedule Declined",
                message=f"Your reschedule request for session with {coach.first_name} {coach.last_name} has been declined. Session remains at original time.",
                notification_type='session',
                related_id=session.id,
                related_type='session'
            )
        ...
        logger.error(f"Error creating reschedule notification: {e}")

def create_reschedule_message_notification(session, notification_type, student, coach):
    """
    Create message notifications for reschedule events in the chat
    ...
    try:
        ...
        # Format session time
        session_time = session.scheduled_at.strftime("%B %d, %Y at %I:%M %p") if session.scheduled_at else "TBD"
        ...
        if notification_type == 'reschedule_requested':
            ...
            message_data = {
                ...
                "duration": str(getattr(session, 'duration_minutes', 60)),
                "status": "reschedule_requested",
                "coach_name": f"{coach.first_name} {coach.last_name}",
                "student_name": f"{student.first_name} {student.last_name}",
                "reason": getattr(session, 'reschedule_reason', 'No reason provided'),
                "requested_at": datetime.now().strftime("%B %d, %Y at %I:%M %p")
            }
            
            message = Message(
                sender_id=student.id,  # Student sends the request
            ...
            message_data = {
                ...
                "coach_name": f"{coach.first_name} {coach.last_name}",
                "student_name": f"{student.first_name} {student.last_name}",
                "approved_at": datetime.now().strftime("%B %d, %Y at %I:%M %p")
            }
            
            message = Message(
                sender_id=coach.id,  # Coach sends the notification
                recipient_id=student.id,
                content=json.dumps(message_data),
                message_type='RESCHEDULE_APPROVED',
                sender_role='coach',
            ...
            message_data = {
                ...
                "coach_name": f"{coach.first_name} {coach.last_name}",
                "student_name": f"{student.first_name} {student.last_name}",
                "declined_at": datetime.now().strftime("%B %d, %Y at %I:%M %p")
            }
        ...
        logger.info(f"Created reschedule {notification_type} message for session {session.id}")
        return message
        
    except Exception as e:
        logger.error(f"Error creating reschedule message notification: {e}")
        return None

def create_meeting_link_notification(session, meeting_url):
    """
    Create a meeting link notification message in the chat between student and coach
    """
    try:
        import json
        from datetime import datetime
        ...
        # Create meeting link notification message
        message_data = {
            "session_id": session.id,
            "session_number": getattr(session, 'session_number', '1'),
            "scheduled_at": session.scheduled_at.strftime("%B %d, %Y at %I:%M %p") if session.scheduled_at else "TBD",
            "duration": str(getattr(session, 'duration_minutes', 60)),
            "status": session.status,
            "meeting_url": meeting_url,
            "coach_name": f"{coach.first_name} {coach.last_name}",
            "student_name": f"{student.first_name} {student.last_name}",
            "meeting_created_at": datetime.now().strftime("%B %d, %Y at %I:%M %p")
        }
        ...
        message = Message(
            ...
            recipient_id=student.id,
            content=json.dumps(message_data),
            message_type='MEETING_LINK_READY',
            sender_role='coach',
            recipient_role='student'
        )
        ...
        logger.info(f"Created meeting link notification message for session {session.id}")
        return message
        
    except Exception as e:
        logger.error(f"Error creating meeting link notification: {e}")
        return None
