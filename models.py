...
class User(db.Model):
    ...
    # Google Meet relationships
    created_scheduled_sessions = db.relationship('ScheduledSession', foreign_keys='ScheduledSession.meeting_created_by', backref='meeting_creator')
    created_scheduled_calls = db.relationship('ScheduledCall', foreign_keys='ScheduledCall.meeting_created_by', backref='meeting_creator')
...
class Proposal(db.Model):
    ...
    def create_contract(self, **contract_data):
        ...
        contract_number = Contract._generate_contract_number()
    ...
    def _generate_contract_number():
        ...
        last_contract = Contract.query.order_by(Contract.id.desc()).first()
        ...
        try:
            # Handle both old format (CTR-20250828-0001) and new format (#001)
            if last_contract.contract_number.startswith('#'):
                last_num = int(last_contract.contract_number[1:])
            else:
                # Extract from old format CTR-20250828-0001
...
class Session(db.Model):
    ...
    reschedule_proposed_time = db.Column(db.DateTime, nullable=True)  # Proposed new time for reschedule
    # Note: reschedule_status column will be added via migration
    ...
    def can_request_reschedule(self, user_role):
        """Check if user can request reschedule based on 5-hour policy"""
        ...
        # Both student and coach can request reschedule
        return True
    
    def is_within_5_hours(self):
        """Check if session is within 5 hours of scheduled time"""
        if not self.scheduled_at:
            return False
        
        cutoff_time = self.scheduled_at - timedelta(hours=5)
        return datetime.utcnow() >= cutoff_time
    
    def request_reschedule(self, requested_by, reason, new_scheduled_at=None):
        """Request a reschedule with 5-hour policy"""
        ...
        # Check if within 5 hours - requires coach approval
        if self.is_within_5_hours():
            # Store the reschedule request for coach approval
            ...
            if new_scheduled_at:
                self.reschedule_proposed_time = new_scheduled_at
            
            db.session.commit()
            return "pending_approval"
        else:
            # More than 5 hours - automatic approval
            if new_scheduled_at:
                ...
                self.reschedule_proposed_time = None
                
                db.session.commit()
                return "auto_approved"
            else:
                # If no new time provided, still require coach approval for safety
                self.reschedule_requested = True
                self.reschedule_requested_by = requested_by
                self.reschedule_reason = reason.strip()
    ...
    def approve_reschedule(self, new_scheduled_at=None):
        """Approve reschedule request (coach only)"""
        print(f"DEBUG: Session.approve_reschedule called with new_scheduled_at: {new_scheduled_at}")
        print(f"DEBUG: Current reschedule_requested: {self.reschedule_requested}")
        print(f"DEBUG: Current reschedule_requested_by: {self.reschedule_requested_by}")
        print(f"DEBUG: Current reschedule_proposed_time: {self.reschedule_proposed_time}")
        
        if not self.reschedule_requested:
            raise ValueError("No reschedule request pending")
        
        # Only coaches can approve reschedule requests
        if self.reschedule_requested_by == 'coach':
            raise ValueError("Coaches cannot approve their own reschedule requests")
        
        # Use provided time or student's proposed time
        if new_scheduled_at:
            print(f"DEBUG: Using provided time: {new_scheduled_at}")
            self.scheduled_at = new_scheduled_at
        elif self.reschedule_proposed_time:
            print(f"DEBUG: Using student's proposed time: {self.reschedule_proposed_time}")
            self.scheduled_at = self.reschedule_proposed_time
        else:
            # If no proposed time, just approve without changing time
            print("DEBUG: No proposed time available - approving without time change")
        
        print(f"DEBUG: Clearing reschedule fields")
        self.reschedule_requested = False
        self.reschedule_requested_by = None
        self.reschedule_reason = None
        self.reschedule_deadline = None
        self.reschedule_proposed_time = None
        # Note: reschedule_status will be set when column exists
    ...
    def decline_reschedule(self):
        """Decline reschedule request (coach only)"""
        if not self.reschedule_requested:
            raise ValueError("No reschedule request pending")
        
        # Only coaches can decline reschedule requests
        if self.reschedule_requested_by == 'coach':
            raise ValueError("Coaches cannot decline their own reschedule requests")
    ...
    def should_be_completed(self):
        """Check if session should be automatically marked as completed"""
        if self.status not in ['scheduled', 'active']:
            return False
        
        if not self.scheduled_at or not self.duration_minutes:
            return False
        
        # Calculate when the session should end
        session_end_time = self.scheduled_at + timedelta(minutes=self.duration_minutes)
        return datetime.utcnow() >= session_end_time
    
    def auto_complete_if_needed(self):
        """Automatically mark session as completed if time has passed"""
    ...
    def get_button_state(self, user_role):
        """Get the appropriate button state for the session"""
        # First, check if session should be auto-completed
        was_completed = self.auto_complete_if_needed()
        
        # If session was just completed, refresh the contract data
        if was_completed:
            contract = self.get_contract()
            if contract:
                # Refresh the contract from database to get updated completed_sessions
        ...
        contract = self.get_contract()
        if contract:
            scheduled_session = contract.get_scheduled_session(self.session_number)
            if scheduled_session and scheduled_session.google_meet_url:
                has_meeting_link = True
        
        if user_role == 'coach':
            return self._get_coach_button_state(has_meeting_link)
        else:
            return self._get_student_button_state(has_meeting_link)
    
    def _get_coach_button_state(self, has_meeting_link):
        """Get button state for coach"""
        ...
        # Has meeting link
        if self.status == 'active' or self.can_join_early():
            # Meeting time has come - show both edit and join
            return {
                'type': 'dual_buttons',
                'edit_button': {
                    'text': 'Edit Meeting',
            ...
            # Meeting link exists but time hasn't come - Edit Meeting only
            return {
                'type': 'edit',
                'text': 'Edit Meeting',
                'url': 'meeting_setup',
    ...
    def _get_student_button_state(self, has_meeting_link):
        """Get button state for student"""
        if self.status == 'completed':
            return {
                'type': 'completed',
                'text': 'Meeting Completed',
...
class Contract(db.Model):
    ...
    def _generate_contract_number():
        """Generate unique contract number"""
        # Find the highest existing contract number with new format (#001, #002, etc.)
        last_new_format_contract = Contract.query.filter(
            Contract.contract_number.like('#%')
        ).order_by(Contract.id.desc()).first()
        
        if not last_new_format_contract:
            # No contracts with new format yet, start from #001
            return "#001"
        
        # Extract number from last new format contract
        try:
            last_num = int(last_new_format_contract.contract_number[1:])
        except (ValueError, IndexError):
            last_num = 0
    ...
    def get_active_sessions(self):
        """Get only active sessions (excluding cancelled ones) for scheduling purposes"""
        sessions = Session.query.filter_by(proposal_id=self.proposal_id).filter(Session.status != 'cancelled').all()
    ...
    def get_scheduled_session(self, session_number):
        ...
        # Find the session with the given session_number from the proposal's sessions
        target_session = None
        for session in self.proposal.sessions:
        ...
        # Now find the ScheduledSession for this session
        return ScheduledSession.query.filter_by(session_id=target_session.id).first()
...
class ScheduledSession(db.Model):
    ...
    google_meet_url = db.Column(db.String(500))  # Google Meet meeting link
    meeting_status = db.Column(db.String(20), default='pending')  # pending, created, active, completed
    meeting_created_at = db.Column(db.DateTime)  # When meeting was created
    meeting_created_by = db.Column(db.Integer, db.ForeignKey('user.id'))  # Who created the meeting
    meeting_notes = db.Column(db.Text)  # Any notes about the meeting
